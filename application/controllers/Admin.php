...
public function create_customer()
{
    ...
    // Optional: validate phone number starts with 06 or 07 and has 10 digits
...
// 		       exit();
    ...
    // Validation failed or first time load
    $this->customer_details(); // Note: this function expects a $customer_id. Could cause error if not handled.
}
...
public function create_sponser($customer_id, $comp_id) {
    ...
    // Load existing guarantor if any
    $existing_guarantor = $this->db
    ...
    $data['guarantor'] = $existing_guarantor;
 ...
 public function delete_customerData($customer_id)
{
 ...
 public function kitini()
 {
	 ...
	 $comp_id = $this->session->userdata('comp_id');
	 $empl_data = $this->queries->get_employee_data($empl_id);
	 $data['grouped_payments'] = $this->queries->get_grouped_payments_by_company($comp_id);
 
	 
	//  echo "<pre>";
	//  print_r($data['grouped_payments']);
	//  exit();
 ...
 public function prev_kitini()
 {
	...
	// Fetch grouped payments based on the provided company ID and date range
	$data['grouped_payments'] = $this->queries->get_grouped_payments_by_company($comp_id, $data['from'], $data['to']);

	// Check if grouped payments are empty
	if (empty($data['grouped_payments'])) {
 ...
 public function print_kitini_transaction()
 {
	 ...
	 // Load view for PDF content - pass data including grouped_payments
